...
from app.repository import upsert_job, save_match
...
st.caption("CV-driven job matching • 24-hour freshness • duplicate protection")
    ...
    if cv and st.button("Process CV", type="primary", use_container_width=True):
        ...
        if cv_mode == "Create profile":
            ...
            if not p:
                ...
                updated, diff = update_profile(profile_id, parsed, cv.name)
                st.session_state.profile_id = profile_id
...
if st.button("Run Job Search Now", type="primary"):
    with st.spinner("Collecting, filtering, deduplicating and scoring jobs..."):
        try:
            result = collect_daily(configured_collectors(), target_count=10, candidate=profile)
...
if result:
    st.write(f"**New jobs found:** {result.get('count', len(result.get('jobs', [])))}")
    for job in result.get("jobs", []):
        score = job.get("overall_score", job.get("score", "—"))
        ...
        with st.container(border=True):
            ...
            with b:
                if job.get("url"):
                    st.link_button("Apply / Open", job["url"], use_container_width=True)

# Stored matches / history
