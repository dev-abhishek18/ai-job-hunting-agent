...
def upsert_job(job: dict):
    ...
    if existing:
        conn.execute(
            """UPDATE jobs SET last_seen_at=?, canonical_url=?, title=?, company=?, location=?,
    ...
    cur = conn.execute(
        """INSERT INTO jobs (fingerprint, canonical_url, title, company, location, work_mode, description,
...
def save_match(job_id: int, result: dict):
    conn = get_connection()
    conn.execute(
        """INSERT INTO job_matches (job_id, role_score, skill_score, experience_score, location_score,
...
def get_new_jobs(limit: int = 20, candidate_id: int | None = None):
    conn = get_connection()
