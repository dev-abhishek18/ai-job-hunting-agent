...
app = FastAPI(
    ...
    version="0.6.0",
    description="CV-driven personalized job hunting agent.",
)
...
def root():
    return {
        "agent": "AI Job Hunting Agent",
        "version": "0.6.0",
        "status": "running",
        "features": [
            "CV profiles", "CV versioning", "profile-driven matching",
            "candidate-specific job history", "24-hour freshness filtering",
            "duplicate protection", "multi-source collectors",
        ],
    }
...
def _parse_upload(data):
    with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
        tmp.write(data)
        return parse_cv(tmp.name)


@app.post("/profiles/from-cv")
async def create_profile_from_cv(file: UploadFile = File(...)):
    if not file.filename.lower().endswith(".pdf"):
        raise HTTPException(400, "Only PDF CV files are supported")
    parsed = _parse_upload(await file.read())
...
def ingest_job(job: JobIn, profile_id: int | None = None):
    ...
    if profile_id is not None:
        ...
        if not candidate:
            raise HTTPException(404, "Profile not found")
        result = score_job(data, candidate)
        save_match(row["id"], result)
        attach_job(profile_id, row["id"])
...
def profile_jobs(profile_id: int, limit: int = 20):
    if not get_profile(profile_id):
        raise HTTPException(404, "Profile not found")
    jobs = candidate_jobs(profile_id, limit)
...
def profile_seen(profile_id: int, job_id: int):
    if not get_profile(profile_id):
        raise HTTPException(404, "Profile not found")
    mark_candidate_seen(profile_id, job_id)
    return {"profile_id": profile_id, "job_id": job_id, "status": "SEEN"}


@app.post("/daily/run")
def daily_run(profile_id: int | None = None):
    candidate = get_profile(profile_id) if profile_id else None
    if profile_id and not candidate:
        raise HTTPException(404, "Profile not found")
    return collect_daily(
